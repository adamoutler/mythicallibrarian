...
if [ ! -f "./librarian" ]; then
 ...
 DownloadML=$(dialog --menu "Download an update first then Configure:" 10 70 15 "Latest" "Download and switch to SVN $svnrev" "Stable" "Download and switch to last stable version"  "Configure"  "using: $lastupdated" 2>&1 >/dev/tty)	
if [ "$?" = "1" ]; then
 	clear
