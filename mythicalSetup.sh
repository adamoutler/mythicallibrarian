...
if [ ! -f "./librarian" ]; then
 ...
 DownloadML=$(dialog --menu "Which version would you like to use?" 10 70 15 "Download and switch to SVN $svnrev" "Stable" "Download and switch to recent stable version" "Latest" "Configure"  "using: $lastupdated" 2>&1 >/dev/tty)	
if [ "$?" = "1" ]; then
 	clear
